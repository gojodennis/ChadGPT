...
module.exports = sansekai = async (client, m, chatUpdate, store) => {
    try {
        ...
        var chad = (typeof m.text == 'string' ? m.text : '')
        ...
        const arg = chad.trim().substring(chad.indexOf(' ') + 1 )
        ...
        let argsLog = (chad.length > 30) ? `${q.substring(0, 30)}...` : chad
        ...
        if (isCmd2) {
            switch(command) { 
                ...
                default:{
                    ...
                    if (isCmd2 && chad.toLowerCase() != undefined) {
                        if (m.chat.endsWith('broadcast')) return
                        if (m.isBaileys) return
                        if (!(chad.toLowerCase())) return
